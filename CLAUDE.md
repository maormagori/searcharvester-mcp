...
- **ACP отключаем целиком на деплойментах, где он гарантированно не работает.** Подтверждено на проде (`searcharvester.magori.xyz`, backend — self-hosted Ollama, `HERMES_INFERENCE_MODEL=gpt-oss:20b`): `hermes acp` эмитит tool-call-shaped **текст**, а не настоящие ACP tool calls — ни разу не дошло до `delegate_task`. Диагностировано через `docker logs`: session file лида показывает только `assistant` chat-content, `tool_calls` пустой. Пока эта модель/бэкенд не эмитит реальные tool calls, каждый `/research`-запрос всё равно спавнит `hermes acp` subprocess и грузит `HERMES_INFERENCE_MODEL` (у нас — ~11 ГБ VRAM на карте с 12 ГБ), безнадёжно ждёт до `_finalize_success`, и только потом падает в `research_fallback`. Это не бесплатно: на однокарточном хосте (RTX 3060 12 ГБ, см. `qwen3:14b`/`qwen3:14b-research` — 40960 context, ниже 64000-порога, который ACP требует для агентных сессий, так что туда их даже не поставить) загрузка `gpt-oss:20b` вытесняет из VRAM всё остальное и рвёт конкурентные запросы к тому же Ollama-инстансу.

  Фикс — `SEARCHARVESTER_ACP_ENABLED=false` (env, см. `docker-compose.yaml` / `main.py::_build_orchestrator`): `Orchestrator._run()` тогда вообще не спавнит `hermes acp`, а сразу уходит в `research_fallback` (`DirectResearchFallback`, `RESEARCH_FALLBACK_MODEL`). При этом `_build_orchestrator()` больше не требует `hermes` на PATH, если ACP выключен. Итоговый статус джобы при выключенном ACP — либо `completed` (fallback нашёл источники), либо сразу `failed` (fallback не нашёл источников) — никогда `degraded`, потому что `degraded` специфичен для "ACP что-то отвечал, но не отчётом", а тут ACP вообще не запускался.

  **TODO, когда будем чинить ACP по-настоящему**: это workaround, не фикс первопричины. Подозреваемая причина — несовместимость GPT-OSS/Harmony chat-template адаптера с ACP wire-протоколом `hermes-agent`'а (модель обучена emit'ить tool calls в Harmony-формате, который парсер ACP, видимо, не распознаёт как настоящий tool_call). Когда будет модель/бэкенд с надёжным ACP tool-calling (или пофиксят Harmony-адаптер), верни `SEARCHARVESTER_ACP_ENABLED=true` (или просто не задавай — дефолт `true`) и пере-протестируй end-to-end на реальном `/research` вызове, не только на unit-тестах.
